...
> ⚠️ **已知限制（FOR 计数器隐式声明）**：Oracle 的 FOR 循环变量（`FOR v IN lo..hi` / `FOR v IN REVERSE lo..hi` 的 `v`）是**隐式声明**的，assemble 只对 varbuf 内已声明变量注入 `DECLARE`。转换产出的 WHILE 框架正确，但 FOR 变量需人工补 `DECLARE v INT DEFAULT <hi 或 lo>;`（forward FOR 初始化=`lo`，REVERSE FOR 初始化=`hi`）。这是 forward/REVERSE FOR 共同的 pre-existing gap，不影响 WHILE/SET 计数器逻辑正确性。建议转换后检查 FOR 变量是否已 DECLARE。
...
- [ ] deferred：FOR 计数器隐式变量自动 DECLARE、LISTAGG OVER 跨行兜底检测
