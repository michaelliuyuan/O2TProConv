...
> ⚠️ **已知限制（FOR 计数器隐式声明）**：Oracle 的 FOR 循环变量（`FOR v IN lo..hi` / `FOR v IN REVERSE lo..hi` 的 `v`）是**隐式声明**的。当源码中 `v` **有显式 DECLARE** 时，assemble 正确处理；当 `v` **无显式 DECLARE**（纯隐式）时，assemble 当前**未注入** `DECLARE v INT DEFAULT <lo/hi>;`，导致 MySQL 运行时报 `Unknown column 'v'`。WHILE 框架与计数器递增/递减逻辑本身正确，仅缺变量声明。建议转换后检查 FOR 变量是否已 DECLARE，或源码规范显式声明 FOR 变量。
...
- [ ] deferred：LISTAGG OVER 跨行兜底检测、FOR 计数器隐式变量自动 DECLARE（Bug #2）
