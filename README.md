...
> **FOR 计数器自动 DECLARE**：Oracle 的 FOR 循环变量（`FOR v IN lo..hi` / `FOR v IN REVERSE lo..hi` 的 `v`）虽是隐式声明，转换器 assemble 阶段会自动注入 `DECLARE v INT DEFAULT <lo 或 hi>;`（forward 初始化=`lo`，REVERSE 初始化=`hi`），无需人工补声明。
