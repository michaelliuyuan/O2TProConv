...
| 2b | `_resolve_rowtype` | P4-a：`%ROWTYPE` 自动展开——`v_name table%ROWTYPE` → 逐字段 DECLARE；代码层 `v_name.col` → `v_name_col`（引号感知）；动态 SQL 字符串内标 TODO；变量名 guard；离线 fallback |
...
- **`%ROWTYPE` 自动展开（P4-a）**：`v_name table%ROWTYPE` → 查 `_schema_columns.tsv` 展开为逐字段 DECLARE（`v_name_col1 TYPE1; v_name_col2 TYPE2; ...`）；body 中代码层 `v_name.col` → `v_name_col`（引号感知替换，不碰字符串字面量）；动态 SQL 字符串内 `v_name.col` → 标 TODO；变量名 guard（≥3 字符 + 非保留词）；无 schema 数据或表未匹配 → 降级 TODO
...
**自动标记 `-- TODO(需人工转换)`**：跨表达式/跨行 `||`（**Route A 下留字面交 PIPES_AS_CONCAT**，TODO 标部署要求——见 Usage Step 5）、未解析的 `%TYPE`（无 schema 数据或列未匹配）、**未解析的 `%ROWTYPE`**（P4-a 已自动展开；表未在 schema 数据中或变量名过短时降级 TODO）、`EXECUTE IMMEDIATE` 的 INTO 子句（MySQL PREPARE 无单行 SELECT 返回）、`LISTAGG OVER(PARTITION BY..)` 分析函数形式（MySQL GROUP_CONCAT 不支持）、**动态 SQL 字符串内的 `%ROWTYPE` 变量引用**（引号感知检测，标 TODO 不替换）、`BULK COLLECT/FORALL`、`TYPE .. IS TABLE OF/RECORD/VARRAY`（PL/SQL 集合/记录类型，MySQL 无对应——BULK COLLECT 依赖的 TYPE 声明也标 TODO）、`TO_CHAR(number/复杂)`、`DBMS_OUTPUT`、内联游标 `FOR rec IN (SELECT..)`、`GOTO`（MySQL 不支持）、嵌套 `DECLARE..BEGIN..END`（含 EXCEPTION 的复杂嵌套块）。
